...
import com.gosystem.commons.adminUsers.dto.UserPermisoValidateDTO;
...
import co.system.out.serverchat.clients.IParametrizacionClientService;
...
public class ClientAdministracionUserImpl implements IAdministracionClientUsers {
	...
	private String pathUserCOnfirmPermiso;
	...
	public void init() {
		...
		this. pathUserCOnfirmPermiso = parametrizacionService.getParametro(KeyParametrosConstantes.PATH_GO_ADMIN_USER_CONFIRM_PERMISO_USER);
	...
	*/
	...
	@Override
	public void confirmPermisoUser(UserPermisoValidateDTO userPermiso) throws HomeException {
		logger.info("METODO : confirmPermisoUser() -> CREANDO USUARIO.... ");
		String urlFull = this.urlGateway+ pathUserCOnfirmPermiso;
		logger.info("METODO : confirmPermisoUser() -> URL : "+urlFull);
		this.restTemplate  = new RestTemplate();;
		try {
			  HttpHeaders headers = new HttpHeaders();
		      headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
              HttpEntity<UserPermisoValidateDTO> request =  new HttpEntity<UserPermisoValidateDTO>(userPermiso, headers);		    
			  ResponseEntity<String> responseEntityStr = restTemplate. postForEntity(urlFull, request, String.class);
			  logger.info("METODO : confirmPermisoUser() -> RESPONSE  : "+ UtilGson.SerializeObjet(responseEntityStr));
			  if( responseEntityStr.getStatusCode() == HttpStatus.ACCEPTED ||  
					  responseEntityStr.getStatusCode() == HttpStatus.CREATED ||
					  responseEntityStr.getStatusCode() == HttpStatus.OK ) {
				  logger.info("METODO : confirmPermisoUser() -> RESPUESTA OK: ");
			  }else {
				  if( Objects.nonNull(responseEntityStr.getBody())  ) {
						throw new HomeException( EntityEnum.USUARIO, MethodsEnum.SAVE, LayerEnum.SERVICE ,responseEntityStr.getBody() );
				  }else {
