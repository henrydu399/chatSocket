...
import co.system.out.serverchat.clients.IUbicacionClienteService;
import java.net.URI;
	...
	public List<CiudadDTO> getAllCiudades() throws HomeException {
		...
		try {
				...
				if ( Objects.nonNull(response)  &&   Objects.nonNull(response.getBody()) ) {
					...
					throw new ParametrizacionException( EntityEnum.CIUDAD, MethodsEnum.GETALL, LayerEnum.CLIENT , ErrorConstantes.GO_PARAMETRIZACION_API_ERROR_CARGANDO_CIUDADES);
				}
		}catch (ParametrizacionException e) {
			logger.severe(e.getMessage());
			throw e;		
	...
	public List<BarrioDTO> getAllBarrios() throws HomeException {
		...
		try {
				...
				if ( Objects.nonNull(response)  &&   Objects.nonNull(response.getBody()) ) {
					...
					throw new ParametrizacionException( EntityEnum.DEPARTAMENTO, MethodsEnum.GETALL, LayerEnum.SERVICE , ErrorConstantes.GO_PARAMETRIZACION_API_ERROR_CARGANDO_BARRIOS);
				}
		}catch (ParametrizacionException e) {
			logger.severe(e.getMessage());
			throw e;		
